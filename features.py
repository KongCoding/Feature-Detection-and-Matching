...
class HarrisKeypointDetector(KeypointDetector):
    ...
    def computeHarrisValues(self, srcImage):
        ...
        g_Ixx = ndimage.gaussian_filter(
            Ix**2, sigma=0.5, truncate=((5 - 1) / 2 - 0.5) / 0.5)
        g_Iyy = ndimage.gaussian_filter(
            Iy**2, sigma=0.5, truncate=((5 - 1) / 2 - 0.5) / 0.5)
        g_Ixy = ndimage.gaussian_filter(
            Ix*Iy, sigma=0.5, truncate=((5 - 1) / 2 - 0.5) / 0.5)
...
class SimpleFeatureDescriptor(FeatureDescriptor):
    def describeFeatures(self, image, keypoints):
        ...
        # maskImage = np.zeros((grayImage.shape[0] + 4, grayImage.shape[1] + 4))
        # maskImage[2:grayImage.shape[0] + 2,
        #           2:grayImage.shape[1] + 2] = grayImage
        ...
        for i, f in enumerate(keypoints):
            ...
            for r in range(-2, 3):
                for c in range(-2, 3):
                    if inbounds(grayImage.shape, [y + r, x + c]):
                        d.append(grayImage[y + r, x + c])
                    else:
                        d.append(0)
            desc[i] = np.array(d)
            # TODO-BLOCK-END
...
class MOPSFeatureDescriptor(FeatureDescriptor):
    def describeFeatures(self, image, keypoints):
        ...
        for i, f in enumerate(keypoints):
            ...
            transMx = np.dot(np.dot(np.dot(t2, s), r), t1)[0:2, [0, 1, 3]]
            # TODO-BLOCK-END
...
class SSDFeatureMatcher(FeatureMatcher):
    def matchFeatures(self, desc1, desc2):
        ...
        for i in range(len(indices)):
            matches.append(cv2.DMatch(i, indices[i], dist[i][indices[i]]))
        # TODO-BLOCK-END
    ...
    def matchFeatures(self, desc1, desc2):
        ...
        if desc2.shape[0] < 2:
            for i in range(desc1.shape[0]):
                matches.append(cv2.DMatch(i, 0, 0))

        dist = spatial.distance.cdist(desc1, desc2, 'euclidean')
        indices = np.argpartition(dist, 2, axis=1)[:, :2]
        # for i in range(len(indices)):
        for i in range(indices.shape[0]):
            d = dist[i][indices[i][1]]
            if d < 1e-5:
                matches.append(cv2.DMatch(i, indices[i][0], 1))
            else:
                matches.append(cv2.DMatch(
                    i, indices[i][0], dist[i][indices[i][0]] / d))
        # TODO-BLOCK-END
